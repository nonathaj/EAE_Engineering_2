...
namespace
{
	...
	int luaCreateDirectoryIfNecessary( lua_State* io_luaState )
	{
		...
		if (lua_isstring(io_luaState, 1))
			...
			return luaL_error(io_luaState, "Argument #2 must be a string (instead of a %s)", luaL_typename(io_luaState, 1));
	...
	int luaOutputErrorMessage( lua_State* io_luaState )
	{
		...
		//// How do you get the error message from the Lua state?
		if (lua_isstring(io_luaState, 1))
		...
		if ( !lua_isnil( io_luaState, 2 ) )
		{
			...
			if (lua_isstring(io_luaState, 2))
				i_errorMessage = lua_tostring(io_luaState, 2);
			else
				return luaL_error(io_luaState, "Argument #1 must be a string (instead of a %s)", luaL_typename(io_luaState, 2));
		}
