...
namespace Lame
{
	...
	Mesh* Mesh::CreateBox(const Lame::Vector3& i_size, const Color32& i_color)
	{
		Mesh* mesh = new Mesh(PrimitiveType::TriangleList);
		if (!mesh)
			return nullptr;
	...
	Mesh* Mesh::CreateSphere(const float i_radius, const size_t i_slice_count, const size_t i_stack_count, const Color32& i_vertex_color)
	{
		Mesh* mesh = new Mesh(PrimitiveType::TriangleList);
		if (!mesh)
			return nullptr;
		...
		//vertices
		{
			mesh->vertices_.reserve((i_stack_count - 1) * i_slice_count + 2);
			mesh->vertices_.push_back(Lame::Vertex(Lame::Vector3(0.0f, i_radius, 0.0f), Lame::Vector2(0.0f, 0.0f), i_vertex_color));
			for (size_t x = 1; x <= i_stack_count; x++)
			{
				...
				for (size_t y = 0; y <= i_slice_count; y++)
				{
					...
					mesh->vertices_.push_back(Lame::Vertex(
			...
			mesh->vertices_.push_back(Lame::Vertex(Lame::Vector3(0.0f, -i_radius, 0.0f), Lame::Vector2(0.0f, 1.0f), i_vertex_color));
		}
		...
		//indices
		{
			for (uint32_t x = 1; x <= i_slice_count; x++)
			{
				...
				mesh->indices_.push_back(x + 1);
				mesh->indices_.push_back(x);
			}
			...
			for (uint32_t i = 0; i < i_stack_count - 1; i++)
			{
				...
				{
					mesh->indices_.push_back(baseIndex + i * ringVertexCount + j);
					mesh->indices_.push_back(baseIndex + i * ringVertexCount + j + 1);
					mesh->indices_.push_back(baseIndex + (i + 1) * ringVertexCount + j);

					mesh->indices_.push_back(baseIndex + (i + 1) * ringVertexCount + j);
					mesh->indices_.push_back(baseIndex + i * ringVertexCount + j + 1);
					mesh->indices_.push_back(baseIndex + (i + 1) * ringVertexCount + j + 1);
				}
			}
			uint32_t southPoleIndex = static_cast<uint32_t>(mesh->vertices_.size() - 1);
			baseIndex = southPoleIndex - ringVertexCount;
			for (uint32_t i = 0; i < i_slice_count; i++)
			{
				mesh->indices_.push_back(southPoleIndex);
				mesh->indices_.push_back(baseIndex + i);
				mesh->indices_.push_back(baseIndex + i + 1);
			}
		}
	...
	Mesh* Mesh::CreateCylinder(const float i_bottom_radius, const float i_top_radius, const float i_height, const float i_slice_count, const float i_stack_count, const Color32& i_vertex_color)
	{
		Mesh* mesh = new Mesh(PrimitiveType::TriangleList);
		if (!mesh)
			return nullptr;
		...
		//Cylinder
		{
			...
			for (int i = 0; i < ringCount; i++)
			{
				...
				for (int j = 0; j <= i_slice_count; j++)
				{
					...
					mesh->vertices_.push_back(Lame::Vertex(v, uv, i_vertex_color));
			...
			for (uint32_t i = 0; i < i_stack_count; i++)
			{
				for (uint32_t j = 0; j < i_slice_count; j++)
				{
					mesh->indices_.push_back(i*ringVertexCount + j);
					mesh->indices_.push_back((i + 1)*ringVertexCount + j);
					mesh->indices_.push_back((i + 1)*ringVertexCount + j + 1);

					mesh->indices_.push_back(i*ringVertexCount + j);
					mesh->indices_.push_back((i + 1)*ringVertexCount + j + 1);
					mesh->indices_.push_back(i*ringVertexCount + j + 1);
				}
			}
		}
		...
		//top
		{
			const uint32_t baseIndex = static_cast<uint32_t>(mesh->vertices_.size());
			...
			for (int i = 0; i <= i_slice_count; i++)
			{
				...
				mesh->vertices_.push_back(Lame::Vertex(Lame::Vector3(x, y, z), Lame::Vector2(u, v), i_vertex_color));
			}
			mesh->vertices_.push_back(Lame::Vertex(Lame::Vector3(0, y, 0), Lame::Vector2(0.5f, 0.5f), i_vertex_color));
			const uint32_t centerIndex = static_cast<uint32_t>(mesh->vertices_.size() - 1);
			for (uint32_t i = 0; i < i_slice_count; i++)
			{
				mesh->indices_.push_back(centerIndex);
				mesh->indices_.push_back(baseIndex + i + 1);
				mesh->indices_.push_back(baseIndex + i);
			}
	...
	Mesh* Mesh::CreateQuad(const Lame::Vector2& i_extends, const Color32& i_vertex_color)
	{
		Mesh* mesh = new Mesh(PrimitiveType::TriangleStrip);
		if (!mesh)
			return nullptr;
